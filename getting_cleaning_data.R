...
offense_year<- function(year){
  ...
  official_standings$Tm[official_standings$Tm=="St Louis Cardinals"]<-"St. Louis Cardinals"
  official_standings$Tm[official_standings$Tm=="St Louis Rams"]<-"St. Louis Rams"

  
  #View(passing_offense)
  #View(rushing_offense)
  #View(kickNpunt_offense)
  #View(official_standings)
  ...
  team_no[i]<-nrow(offense_data2)
  if(year<length(range)){
  i<-i+1
  }
  ##team_year
...
defense_year<-function(year){
  ...
  #View(passing_defense)
  #View(rushing_defense)
  #View(kickNpunt_defense)
...
View(NFL.combined)
team_no[0:length(range)]

print(paste("I have scrapped NFL data from :", start, "to", end))
print(paste("There are a total of", sum(team_no), "readings"))
write.csv(NFL.combined, file ="NFL_with_standings")
